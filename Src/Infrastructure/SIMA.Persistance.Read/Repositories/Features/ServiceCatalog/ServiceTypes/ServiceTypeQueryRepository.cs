...
public class ServiceTypeQueryRepository : IServiceTypeQueryRepository
{
    ...
    public async Task<Result<IEnumerable<GetServiceTypesQueryResult>>> GetAll(GetAllServiceTypesQuery request)
    {
        ...
        var response = await multi.ReadAsync<GetServiceTypesQueryResult>();
        return Result.Ok(response, request, count);
    }

    public async Task<GetServiceTypesQueryResult> GetById(GetServiceTypeQuery request)
    {
        var query = @"
          SELECT AT.[Id]
              ,AT.[Name]
              ,AT.[Code]
	          ,A.[Name] ActiveStatus
          FROM [ServiceCatalog].[ServiceType] AT
          INNER JOIN [Basic].[ActiveStatus] A ON AT.ActiveStatusId = A.ID
          WHERE AT.[Id] = @Id AND AT.ActiveStatusId <> 3";
        using var connection = new SqlConnection(_connectionString);
        await connection.OpenAsync();
        var result = await connection.QueryFirstAsync<GetServiceTypesQueryResult>(query, new { request.Id });
        result.NullCheck();
