...
,rh.Description
from ServiceCatalog.Api a
join ServiceCatalog.ApiRequestHeaderParam rh on a.Id=rh.Id and rh.ActiveStatusId<>3
...
,rb.Description
from ServiceCatalog.Api a
join ServiceCatalog.ApiRequestBodyParam rb on a.Id=rb.Id and rb.ActiveStatusId<>3
...
,rh2.Name Parent
from ServiceCatalog.Api a
join ServiceCatalog.ApiResponseHeaderParam rh on a.Id=rh.Id and rh.ActiveStatusId<>3
...
,rb2.Name Parent
from ServiceCatalog.Api a
join ServiceCatalog.ApiResponseBodyParam rb on a.Id=rb.Id and rb.ActiveStatusId<>3
...
,u2.Name Parent
from ServiceCatalog.Api a
join ServiceCatalog.ApiRequestUrlParam u on a.Id=u.Id and u.ActiveStatusId<>3
...
,s2.Name Parent
from ServiceCatalog.Api a
join ServiceCatalog.ApiRequestQueryStringParam s on a.Id=s.Id and s.ActiveStatusId<>3
...
where a.ActiveStatusId <> 3 and a.[Id] = @Id";
        ...
        response.ApiDocumentList = (await multi.ReadAsync<ApiDocumentQueryResult>()).ToList();
        response.ApiSupportTeamList = (await multi.ReadAsync<ApiSupportTeamQueryResult>()).ToList();
        response.ConfigurationItemApiList = (await multi.ReadAsync<ConfigurationItemApiQueryResult>()).ToList();
        response.ApiRequestHeaderParamList = (await multi.ReadAsync<ApiRequestHeaderParamQueryResult>()).ToList();
        response.ApiRequestBodyParamList = (await multi.ReadAsync<ApiRequestBodyParamQueryResult>()).ToList();
        response.ApiResponseHeaderParamList = (await multi.ReadAsync<ApiResponseHeaderParamQueryResult>()).ToList();
        response.ApiResponseBodyParamList = (await multi.ReadAsync<ApiResponseBodyParamQueryResult>()).ToList();
        response.ApiRequestUrlParamList = (await multi.ReadAsync<ApiRequestUrlParamQueryResult>()).ToList();
        response.ApiRequestQueryStringParamList = (await multi.ReadAsync<ApiRequestQueryStringParamQueryResult>()).ToList();
