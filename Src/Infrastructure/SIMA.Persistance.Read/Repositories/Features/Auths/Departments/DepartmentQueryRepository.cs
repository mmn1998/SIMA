...
";
    ...
    public async Task<Result<List<GetDepartmentQueryResult>>> GetAll(BaseRequest? baseRequest = null)
    {
        var response = new List<GetDepartmentQueryResult>();
        int totalCount = 0;
        if (baseRequest != null && !string.IsNullOrEmpty(baseRequest.SearchValue))
        {
            using (var connection = new SqlConnection(_connectionString))
            {
              ...
              on D.ActiveStatusId = a.ID
              LEFT JOIN [Organization].[Company] C on C.ID = D.CompanyID AND C.[ActiveStatusID] <> 3
                WHERE (D.[Name] like @SearchValue OR C.[Name] like @SearchValue OR D.[Code] like @SearchValue) AND D.[ActiveStatusID] <> 3
Order By d.[CreatedAt] desc
";
                var result = await connection.QueryAsync<GetDepartmentQueryResult>(query, new { SearchValue = "%" + baseRequest.SearchValue + "%" });
...
Order By d.[CreatedAt] desc
";
                var result = await connection.QueryAsync<GetDepartmentQueryResult>(query);
                totalCount = result.Count();
                response = result.Skip((baseRequest.Take - 1) * baseRequest.Skip).Take(baseRequest.Take).ToList();
            }
        }
