...
namespace SIMA.Domain.Models.Features.RiskManagement.TriggerStatuses.Entities;

public class TriggerStatus : Entity, IAggregateRoot
{
    private TriggerStatus()
    {

    }
    private TriggerStatus(CreateTriggerStatusArg arg)
    {
        Id = new(arg.Id);
        Name = arg.Name;
        Code = arg.Code;
    ...
    public static async Task<TriggerStatus> Create(CreateTriggerStatusArg arg, ITriggerStatusDomainService service)
    {
        await CreateGuadrs(arg, service);
        return new TriggerStatus(arg);
    }
    public async Task Modify(ModifyTriggerStatusArg arg, ITriggerStatusDomainService service)
    {
    ...
    public TriggerStatusId Id { get; private set; }
    ...
    #region Guards
    private static async Task CreateGuadrs(CreateTriggerStatusArg arg, ITriggerStatusDomainService service)
    {
        arg.NullCheck();
        arg.Name.NullCheck();
        arg.Code.NullCheck();
    ...
    private async Task ModifyGuard(ModifyTriggerStatusArg arg, ITriggerStatusDomainService service)
    {
        arg.NullCheck();
        arg.Name.NullCheck();
        arg.Code.NullCheck();
