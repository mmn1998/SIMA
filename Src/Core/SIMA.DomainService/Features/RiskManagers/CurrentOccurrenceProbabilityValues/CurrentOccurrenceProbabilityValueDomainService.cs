...
public class CurrentOccurrenceProbabilityValueDomainService : ICurrentOccurrenceProbabilityValueDomainService
{
    ...
    public async Task<bool> IsNumericUnique(float value, CurrentOccurrenceProbabilityValueId? id = null)
    {
        ...
        bool result = false;
        if (id == null) result = !await _context.CurrentOccurrenceProbabilityValues.AnyAsync(x => x.NumericValue == value && x.ActiveStatusId == (long)ActiveStatusEnum.Active);
        else result = !await _context.CurrentOccurrenceProbabilityValues.AnyAsync(x => x.NumericValue == value && x.Id != id && x.ActiveStatusId == (long)ActiveStatusEnum.Active);
        return result;
    }
}