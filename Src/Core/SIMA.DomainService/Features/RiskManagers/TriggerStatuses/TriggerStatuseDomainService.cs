...
public class TriggerStatuseDomainService : ITriggerStatusDomainService
{
    ...
    public async Task<bool> IsNumericUnique(float value, TriggerStatusId? id = null)
    {
        ...
        bool result = false;
        if (id == null) result = !await _context.TriggerStatuses.AnyAsync(x => x.NumericValue == value && x.ActiveStatusId == (long)ActiveStatusEnum.Active);
        else result = !await _context.TriggerStatuses.AnyAsync(x => x.NumericValue == value && x.Id != id && x.ActiveStatusId == (long)ActiveStatusEnum.Active);
        return result;
    }
}