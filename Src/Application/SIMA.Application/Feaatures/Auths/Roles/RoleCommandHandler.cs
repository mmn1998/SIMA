...
public class RoleCommandHandler : ICommandHandler<DeleteRoleCommand, Result<long>>, ICommandHandler<CreateRoleAggregate, Result<long>>,
    ICommandHandler<UpdateRoleCommand, Result<long>>
    /*ICommandHandler<CreateRoleCommand, Result<long>>,*/
    /*, ICommandHandler<CreateRolePermissionCommand, Result<long>>,ICommandHandler<UpdateRolePermissionCommand, Result<long>>,*/
    /*ICommandHandler<DeleteRolePermissionCommand, Result<long>>,*/

{
    ...
    public async Task<Result<long>> Handle(CreateRoleAggregate request, CancellationToken cancellationToken)
    {
        ...
        if (request.FormRoles != null && request.FormRoles.Count != 0)
        {
            var formRolesArg = _mapper.Map<List<CreateFormRoleArg>>(request.FormRoles);
            foreach (var item in formRolesArg)
            {
                item.CreatedBy = _simaIdentity.UserId;
                item.RoleId = entity.Id.Value;
            }

            await entity.AddFormRole(formRolesArg, entity.Id.Value, _roleService);
        }

        if (request.RoleUsers != null && request.RoleUsers.Count != 0)
        {
            var args = _mapper.Map<List<CreateUserRoleArg>>(request.RoleUsers);
            foreach (var RoleArg in args)
            {
                RoleArg.CreatedBy = _simaIdentity.UserId;
                RoleArg.RoleId = entity.Id.Value;
            }
            await entity.AddUserRole(args, entity.Id.Value);
        }
